...
import { safeCodecEndpoint } from './payloads/codec';
...
function bindToggle(key: BooleanSetting): void {
    ...
    input.addEventListener('change', () => {
        ...
        // EVERY toggle, not just the payload one. Two of these switches decide
        // whether a configured endpoint can send anything at all, so the verdict
        // below is only true if it is recomputed whenever any of them moves.
        showCodecVerdict();
    });
}
...
// ── The codec endpoint ──────────────────────────────────────────────────────
//
// The same two-layer arrangement as a deep-link template, for the same reason: the
// popup says immediately that a value will not be used, and the page world re-checks
// it before every request — because a setting can also be written by another build,
// by storage sync, or by hand, and none of those go through this file.
function bindCodecEndpoint(): void {
    const input = $<HTMLInputElement>('codecEndpoint');
    input.value = settings.codecEndpoint;
    input.addEventListener('change', () => {
        settings.codecEndpoint = input.value;
        showCodecVerdict();
        void saveSettings({ codecEndpoint: input.value });
    });
    showCodecVerdict();
}

// The one piece of UI text in this repository that has to be exactly true.
//
// It answers "will anything leave this machine, and to where?", and the honest
// answer depends on three things, checked in the order that decides the outcome:
// whether the value is usable at all, whether it is empty, and whether the features
// that would use it are switched on. A verdict that names a host while the panel is
// switched off is a false statement about egress in the one place a reader has no
// other way to check.
function showCodecVerdict(): void {
    const input = $<HTMLInputElement>('codecEndpoint');
    const warning = $('codec-warning');
    const accepted = safeCodecEndpoint(input.value);
    const typed = input.value.trim() !== '';

    input.classList.toggle('invalid', typed && accepted === null);
    if (typed && accepted === null) input.setAttribute('aria-invalid', 'true');
    else input.removeAttribute('aria-invalid');

    if (!typed) {
        // Worth stating rather than leaving blank: this field is the ONLY thing that
        // can make this extension send a payload anywhere, so empty is a guarantee
        // and not merely an absence.
        warning.textContent = 'Empty: no payload byte can leave this machine.';
        return;
    }
    if (accepted === null) {
        // Naming the reason matters here: "https, or http on this machine" is a rule
        // about where decrypted payloads may travel, not a formatting nit.
        warning.textContent =
            '⚠ Not used. A codec server returns DECRYPTED payloads, so it must be https:// — or http:// on localhost, where there is no network to listen on.';
        return;
    }
    if (!settings.enabled || !settings.payloadsEnabled) {
        // Kept, not cleared: a user who switches the panel off for an afternoon
        // should not have to retype a URL. But it must not read as if it were live.
        const why = !settings.enabled ? 'the extension is switched off' : 'the payload panel is switched off';
        warning.textContent = `Saved, but nothing is being sent: ${why}.`;
        return;
    }
    // The host, spelled out, and only the payloads that are actually sent. "A codec
    // server is configured" does not look like "encrypted payloads from the rows you
    // hover will be POSTed to codec.example.com" until it is written that way.
    warning.textContent = `Payloads this extension cannot read will be sent to ${new URL(accepted).host} when you hover a row. Readable ones never leave the browser, and your access token is never sent.`;
}
