...
//
// The two payload buttons — "In" and "Out" — the only things a render pass adds
// for the payload panels.
//
// Each is a button and nothing else — no row identity on it, no request behind
// it. Both halves of that are asserted here, because both were true by accident
// before they were true on purpose: a button carrying a workflow id is wrong the
// moment the UI recycles the <tr> under it, and a table of 100 rows must cost
// zero requests until a pointer lands on one (rule 1 at the top of
// src/payloads/tooltip.ts).

import { beforeEach, describe, expect, it } from 'vitest';

import { LINK_CLASS, PAYLOAD_INPUT_CLASS, PAYLOAD_OUTPUT_CLASS, RETRY_CLASS } from '../../src/decoration';
...
describe('the payload buttons', () => {
    it('adds one of each per row and carries no row identity on either', () => {
        ...
        const inputs = Array.from(tbody.querySelectorAll<HTMLButtonElement>(`.${PAYLOAD_INPUT_CLASS}`));
        const outputs = Array.from(tbody.querySelectorAll<HTMLButtonElement>(`.${PAYLOAD_OUTPUT_CLASS}`));
        expect(inputs).toHaveLength(2);
        expect(outputs).toHaveLength(2);
        // type=button: inside a form, a button with no type submits it.
        for (const button of [...inputs, ...outputs]) {
    ...
    it('omits both buttons on a row it has no run id for', () => {
        ...
        applyToTable(tbody, lookupFor(FAMILY), { ...OPTIONS, payloadsEnabled: true });
        expect(tbody.querySelectorAll(`.${PAYLOAD_INPUT_CLASS}`)).toHaveLength(0);
        expect(tbody.querySelectorAll(`.${PAYLOAD_OUTPUT_CLASS}`)).toHaveLength(0);
    });

    it('removes both buttons when the setting is switched off, and puts them back', () => {
        ...
        expect(tbody.querySelectorAll(`.${PAYLOAD_INPUT_CLASS}`)).toHaveLength(1);
        expect(tbody.querySelectorAll(`.${PAYLOAD_OUTPUT_CLASS}`)).toHaveLength(1);
    ...
    it('puts the badge, then the links, then Input, then Out, whichever toggle was used first', () => {
        // The order on screen must not depend on the order the toggles were used in.
        // All four controls are created with appendChild, so this spec fails the
        ...
        //
        // All four, not two: every control that shares the cell has to be in here,
        ...
        const named: ReadonlyArray<readonly [string, string]> = [
            [RETRY_CLASS, 'retry'],
            [PAYLOAD_INPUT_CLASS, 'payload-input'],
            [PAYLOAD_OUTPUT_CLASS, 'payload-output'],
            [LINK_CLASS, 'link'],
        ];
        ...
        const wanted = ['retry', 'link', 'payload-input', 'payload-output'];
    ...
    it('adds exactly one of each button however many passes run', () => {
