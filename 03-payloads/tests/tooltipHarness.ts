// The jsdom harness for the payload panel: one installed panel, one table, and the
// helpers that hover it and answer it.
//
// WHY IT IS SHARED RATHER THAN COPIED. Two spec files drive this panel — one for the
// five rules at the top of src/payloads/tooltip.ts (the element and the gesture), one for the
// four invariants at the top of src/payloads/payloadClient.ts (which answer may be believed)
// — and they must drive the SAME installed panel. Two private harnesses would drift,
// and the drift would be invisible: each file would keep passing against its own
...
// What the page looks like, as the installed panel sees it. Every dependency is read
// through a function, so a spec can change any of these BETWEEN two hovers — which is
// the only way to test that the namespace is part of the cache key.
//
...
// Every payload request the panel posted, in order. Emptied in place rather than
// reassigned, so a spec can hold a reference to it across a reset.
...
export function addSecondButton(row: TooltipRow = SECOND_ROW): HTMLElement {
    const cell = document
        .querySelector('tbody')!
...
export function section(kind: PayloadKind): { heading: Element; body: Element } {
...
export async function hoverAndAnswer(overrides: Partial<PayloadResult> = {}): Promise<void> {
