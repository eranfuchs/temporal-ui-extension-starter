...
// The per-row affordance that opens the payload panel. payloads/payloadButton.ts
// RENDERS it and knows nothing else about it — the hovering, the fetching and the
// panel itself are payloads/tooltip.ts, which finds these buttons by this class.
export const PAYLOAD_CLASS = 'tuis-payload';
// The floating payload panel. payloads/tooltip.ts builds it and render.ts only ever
// removes it (see removeAllDecoration).
