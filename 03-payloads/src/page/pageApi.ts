...
// here too: a codec server allows the Temporal UI's origin, which a content-script
// fetch does not carry either. That call is fetchFromPageWorld() in section 2, and
// it deliberately does NOT go through the gate below, because it carries none of
// the authority the gate protects — see the top of codec.ts for what it may send.
//
...
// reach and receives the DECODED text back on a channel the forger is listening
// to. Neither is a shape check, and a shape check does not substitute for either.
// The full argument is "the weakness, and what closing most of it took" in
// README.md.
//
