...
//
// ── THIS FILE USED TO BE A FLOATING CARD, AND THAT WAS THE WRONG ANSWER ──────
//
// The first version drew one fixed box in the bottom-right corner listing every
// activity it knew about, and its own header argued for that: rule 1 at the top of
// render.ts is ANCHOR TO MEANING, NOT POSITION, the workflow list has a perfect
// anchor in `a[href*="/workflows/"]`, and a workflow's page — so the argument went —
// has no equivalent for "activity N", because the timeline is SVG and the event list
// is virtualised.
//
// The premise was false, and the way it was false is worth keeping written down.
// There IS a meaning-anchor on that page: when the UI opens an activity's panel it
// renders a labelled row whose label is the words "Activity Id" and whose value is
// the id. That is meaning, not position — the label is the UI's own name for the
// field, it survives a restyle, and it is how the internal extension this starter
// came from has anchored per-activity buttons for months. Reasoning from "I cannot
// see an anchor" to "there is no anchor" produced a floating overlay that covered
// the page it was annotating and listed activities the reader had not asked about.
//
// It was worse than clutter. Because nothing on the page could be attached to, the
// card also could not put a link ON an activity, so it fell back to LISTING
// activities and telling the reader to add `{activityType}` to a template if they
// wanted per-activity links. A feature that explains how to configure itself, on
// the page where it could simply have worked, is a feature that has given up; and
// the type it suggested keying on is not an identity (see below), so following the
// instruction produced links that quietly matched the wrong activity.
//
// ── THE FALLBACK IS A WAY-STATION, NOT A DESTINATION ─────────────────────────
//
// Anchoring to the page's own layout has one real failure mode, and the internal
// extension paid for it: when the selectors go stale the node is not missing, it is
// attached to <body> at the top-left, invisible behind the app's chrome. It reads as
// "the feature only appears after you toggle it", and it was diagnosed twice before
// anybody found the cause.
//
// So a bar that cannot find its anchor is parked in a corner where it can be SEEN
// (LINK_BAR_ADRIFT_CLASS), the popup counts that state, and every pass looks for the
// real anchor again and moves the bar there the moment it appears. A single-page app
// renders its header after its first paint, so "adrift" is the normal state for the
// first frame and a bug only if it persists.
//
// ── IT FETCHES NOTHING ───────────────────────────────────────────────────────
//
...
// already open on a workflow when the extension loaded, the bar says it has observed
// nothing and asks for a reload. It does NOT go and fetch the history to fill itself
// in. That is the trade this stage of the starter kit is demonstrating.
...
import { templatesInScope, type DeepLinkTemplate } from './deepLink';
...
// the top of src/rowInfo.ts is the rule, and it applies here for the same reason: a
