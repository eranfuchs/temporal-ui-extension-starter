...
The five non-obvious traps in that paragraph — the `window.fetch` getter lock,
`<tr>` recycling, the self-feeding `MutationObserver`, the connector overlay, and
why `z-index` must be exactly `0` — are documented in
...
`host_permissions` entry, a service worker, a `chrome.*` call in `src/`, an
`@types/chrome` entry in *this project's* `package.json`, a `"chrome"` entry in
its `tsconfig.json` `types`, a runtime dependency, any of the markup/code sinks
(`innerHTML`, `eval`, …), a content-script match outside the allowed hosts, and
`<all_urls>` outright.
`npm run surface:selftest` drives it against a case per rule that it must reject,
because a gate that has quietly stopped detecting its own failure case is worse
than no gate.
...
- **Verified once against live Temporal Cloud** — 2026-08-30, Chrome 151, one
  tenant, one page of 25 workflows. A headed browser-automation probe (which
  lives outside this repository, because it drives a private namespace)
  confirmed each link in the chain separately: the `window.fetch` wrapper
  survived Cloud's bootstrap, Cloud's list URL still matched the expected shape,
  every row on the page was annotated, the four families present were indented
  with connectors drawn, `content.css` was in effect — checked through computed
  style, not by reading the stylesheet — and a second settled render pass wrote
  nothing, which is the idempotency property the specs assert offline.
...
Deep links to your log tool, a settings pane, payload decoding, column reorder,
a bigger page size, a cross-workflow activity finder — those are
[`../02-techniques/`](../02-techniques/) and [`../03-goodies/`](../03-goodies/).
Each step up the ladder costs something: 02 asks for `storage`. This one asks for
nothing, and that is the point of having it separately.
