...
// it — the note inside the link bar, in detail/detailLinks.ts — because removing the
// root takes it with it. A modifier written onto a root is local for the same reason
// (LINK_BAR_ADRIFT_CLASS, same file). The test for a new class: it belongs here
// unless removing something named here already takes it off the page AND exactly one
// file ever writes the string.
//
...
// two nodes that are not in the table at all, the link sites on a single workflow's
// own page, which detail/detailLinks.ts owns and finds by these same names. A selector
// in one file and the node it is meant to find in another is how the two drift apart,
// so each string exists once, here.
...
export const REMOVABLE_ROOT_CLASSES: readonly string[] = [
    ...
    LINK_CLASS,
    RETRY_CLASS,
    // The workflow page's two link sites are here even though neither lives in a row:
    // "off" has to mean off, and they are the most visible thing this extension draws,
    // so leaving one behind is the clearest possible way to look like the master switch
    // does nothing — which is exactly what makes a security reviewer stop believing the
    // rest of the claims. detail/detailLinks.ts rebuilds them on demand.
    LINK_BAR_CLASS,
