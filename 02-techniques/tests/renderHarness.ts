...
//
// WHY IT IS SHARED RATHER THAN COPIED. Three spec files drive applyToTable() —
// render.spec.ts (the table and the tree), renderLinks.spec.ts (the deep-link
// buttons) and renderRowInfo.spec.ts (the two features that cost a request) — and
// they must drive it against the SAME fixtures. A render pass writes the whole row:
// the tree prefix, the link buttons and the two request-backed cells all land in one
// call, and several of the assertions below are about one of them not disturbing
// another. Per-file fixtures that drifted would each keep passing while the
// interaction between them stopped being tested.
//
...
//
// This file is a declared FORK rather than shared, and by one line: 03-payloads'
// OPTIONS carries `payloadsEnabled`, a field this stage's RenderOptions has not got.
